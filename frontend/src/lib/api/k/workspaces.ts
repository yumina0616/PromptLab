...
import type {
  ...
  WorkspacePromptCreatePayload,
  WorkspacePromptCreateResponse,
...
export const createWorkspacePrompt = async (
  workspaceId: number,
  payload: WorkspacePromptCreatePayload
): Promise<WorkspacePromptCreateResponse> => {
  const response = await apiClient.post<WorkspacePromptCreateResponse>(
    `/workspaces/${workspaceId}/prompts`,
    payload
  );
  return response.data;
